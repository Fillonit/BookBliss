...
export default function ContactsStats() {
    ...
    useEffect(() => {
        const urls: { key: string; url: string }[] = [
            ...
            {
                key: 'feedbackCount',
                url: `${API_URL}/api/contact/count?type=feedback`,
            },
            {
                key: 'reportCount',
                url: `${API_URL}/api/contact/count?type=report`,
            },
            {
                key: 'averageUserContact',
                url: `${API_URL}/api/contact/average-user-contact`,
            },
        ]
    ...
    const feedBackPercentage =
        contacts === 0 ? 0 : ((feedbacks / contacts) * 100).toFixed(0)
    const feedBackText = `${feedBackPercentage}% of total contacts (${feedbacks} ${
        feedbacks > 1 ? 'feedbacks' : 'feedback'
    }) are feedbacks`

    const reportPercentage =
        contacts === 0 ? 0 : ((reports / contacts) * 100).toFixed(0)
    const reportText = `${reportPercentage}% of total contacts (${reports} ${
        reports > 1 ? 'reports' : 'report'
    }) are reports`
    return (
