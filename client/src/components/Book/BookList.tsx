...
for (let i = 1; i <= 250; i++) {
  books.push({
    ...
    title: `Book ${i} Title`,
    author: 'Author Name',
    description: 'Description of the book goes here. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla euismod lorem non posuere commodo.',
...
const BookList: React.FC = () => {
  ...
  const genres: string[] = ['Action', 'Animated', 'Adventure', 'Asian', 'Biography', 'Nordic', 'Documentary', 'Drama', 'Erotic +18', 'Family', 'Fantasy', 'Science Fiction']; // Translated genres
  ...
  <div className="flex justify-between items-center">
    ...
    <div className="relative inline justify-self-end">
      <button className="text-black text-white font-bold py-2 px-3 rounded inline-flex items-center" onClick={toggleDropdown}>
        <img src={filterLogo} alt="Filter Logo" className="h-11 w-auto" style={{ filter: 'invert(1)' }} />
      </button>
      {isDropdownOpen && (
  ...
  <div className="flex flex-col md:flex-row">
    <div className="w-full md:w-1/5 p-5 "> {/* Added flex-col to make it a column */}
      {/* Sidebar for genres */}
      <div className="bg-[#242424] p-4 rounded-lg " style={{ marginTop: '210px' }}>
        <h2 className="text-white text-3xl font-bold mb-2 text-center mb-5">Genres</h2> {/* Adjusted font size */}
        <div className="flex flex-col"> {/* Added flex-wrap for genre buttons */}
          {genres.map(genre => (
            <button
              key={genre}
    ...
    <div className="w-full md:w-4/5 p-5">
      <h1 className="text-3xl font-bold my-4 text-white text-center pt-10">Book Listing</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {applyFilters(books)
          .filter(book => book.title.toLowerCase().includes(searchQuery.toLowerCase())) // Filter books based on search query
          .slice(0, visibleBooks)
          .map(book => (
            <div key={book.id} className="relative bg-white rounded-lg overflow-hidden">
              <img src={book.image} alt={book.title} className="w-full h-100 object-cover" />
              <div className="absolute bottom-0 left-0 right-0 p-4 text-white bg-black bg-opacity-80 opacity-0 transition-opacity duration-300 hover:opacity-100">
                ...
                <p className="text-white-600 mb-2">{book.author}</p>
                <p className="text-white-700">{book.description}</p>
              </div>
            </div>
          ))}
      </div>
      {visibleBooks < books.length && (
        <div className="flex justify-center my-4">
          <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" onClick={handleShowMore}>Show More</button>
        </div>
      )}
    </div>
  </div>
