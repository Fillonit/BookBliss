...
export const createBook = async (req: express.Request, res: express.Response) => {
	...
	const coverFile = Array.isArray(req.files) ? req.files[0].filename : req.files["cover"][0].filename;
	const pdfFile = Array.isArray(req.files) ? req.files[0].filename : req.files["cover"][0].filename;
	...
	await prisma.book.create({
		data: {
			...
			isbn: (Math.floor(Math.random() * 9000000000000) + 1000000000000).toString(),
